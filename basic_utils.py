...
def get_videos_in_directory(folder_path):
    ...
    return files
def load_video(lmdb_path,vid):
    video_visual_env = lmdb.open(lmdb_path, readonly=True, create=False, max_readers=4096 * 8,
                                          readahead=False)
    video_visual_txn = video_visual_env.begin(buffers=True)
    dump = video_visual_txn.get(vid.encode())
