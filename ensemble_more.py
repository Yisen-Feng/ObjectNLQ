...
if __name__ == '__main__':
    ...
    # filename1='/root/autodl-tmp/model/GroundNLQ/ckpt/ego4d_nlq_v2_object_attention_1e-4_objectattn/nlq_predictions_epoch_val_top10_3.json'
    # filename2='/root/autodl-tmp/model/GroundNLQ/ckpt/ego4d_nlq_v2_object_attention_base+large_1e-4_objectattn4/nlq_predictions_epoch_val_top10_1.json'
    # filename3='/root/autodl-tmp/model/GroundNLQ/ckpt/ego4d_nlq_v2_pretrain_finetune_internvideo_1e-4_finetune_1gpu20/nlq_predictions_epoch_val_top10_2.json'

    filename1='/root/autodl-tmp/model/GroundNLQ/ckpt/ego4d_nlq_v2_object_encoderca_1e-4_split1_objectattn50/nlq_predictions_epoch_val_top10_2.json'
    filename2='/root/autodl-tmp/model/GroundNLQ/ckpt/ego4d_nlq_v2_object_encoderca_1e-4_split2_objectattn51/nlq_predictions_epoch_val_top10_2.json'
    filename3='/root/autodl-tmp/model/GroundNLQ/ckpt/ego4d_nlq_v2_object_encoderca_1e-4_objectattn44/nlq_predictions_epoch_val_top10_2.json'
    filename4='/root/autodl-tmp/model/GroundNLQ/ckpt/ego4d_nlq_v2_object_encoderca_1e-4_split0_objectattn49/nlq_predictions_epoch_val_top10_2.json'
    filename5='/root/autodl-tmp/model/GroundNLQ/ckpt/ego4d_nlq_v2_object_encoderca_1e-4a_split3_objectattn52/nlq_predictions_epoch_val_top10_2.json'
    filenames=[filename1,filename2,filename3,filename4,filename5]
    # read each post-model prediction
    predictions=[]
    for filename in filenames:
        prediction = load_json(filename)["results"]
        predictions.append(prediction)
    queries_num=len(predictions[0])
    fusion_results = []
    for prediction_idx  in range(queries_num):

        # first, ensemble the top1 prediction of three models to make the final top1 prediction
        top1_generator_list = []
        for prediction in predictions:
            top1_generator_list.extend(prediction[prediction_idx]["predicted_times"][:top1_max_input])
        del prediction
        ...
        fusion_output = copy.deepcopy(predictions[0][prediction_idx])
        fusion_output["predicted_times"] = fusion_output["predicted_times"][:max_input]
        for i in range(len(predictions)):
            if i==0:
                continue
            fusion_output["predicted_times"].extend(predictions[i][prediction_idx]["predicted_times"][:max_input])
    ...
    dataset='nlq'
    submission_path = "ensemble_{}_{}.json".format(dataset,"test_final")
    if dataset=='nlq':
