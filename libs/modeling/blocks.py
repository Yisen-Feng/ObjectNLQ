...
class LinearLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        return x  # (N, L, D)
class zeroLinearLayer(LinearLayer):
    def __init__(self, in_hsz, out_hsz, layer_norm=True, dropout=0.1, relu=True):
        super(zeroLinearLayer, self).__init__(in_hsz, out_hsz, layer_norm, dropout, relu)
        with torch.no_grad():
            self.net[1].weight.zero_()  # 将权重参数设为零
            if self.net[1].bias is not None:
                self.net[1].bias.zero_()  # 如果有偏置参数，也将其设为零
...
class zeroMaskedConv1D(MaskedConv1D):
    """
    Masked 1D convolution. Interface remains the same as Conv1d.
    Only support a sub set of 1d convs
    """
    ...
    def __init__(
        ...
        super(zeroMaskedConv1D,self).__init__(
            **kwargs)
...
class ObjectMaskedMHAnew(MaskedMHA):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        # torch.nn.init.constant_(self.key.weight, 0.)#即使你赋值为0，也没有用，因为softmax终归会有值
        # torch.nn.init.constant_(self.key.bias, 0.)
        # torch.nn.init.constant_(self.value.weight, 0.)
    ...
    def forward(self,x, mask, encoder_hidden_states, encoder_hidden_states_mask):
        ...
        q = q.view(B,1, self.n_head, self.n_channels, T).permute(0,2,4,1,3)#[bs,n_h,T,1,c_h]
        k = k.view(B, -1,self.n_head, self.n_channels, T).permute(0,2,4,1,3)#[bs,n_h,T,maxobjlen,c_h]
        v = v.view(B, -1,self.n_head, self.n_channels, T).permute(0,2,4,1,3)#[bs,n_h,T,maxobjlen,c_h]
        q=q*self.scale

        att = torch.matmul(q,k.transpose(-2,-1))#[bs,n_h,T,1,O]
        att_mask=encoder_hidden_states_mask.view(B,-1,1,1,T)#[bs,obj,1,1,T]
        att_mask=att_mask.permute(0,2,4,3,1)#[bs,1,T,1,O]
        att = att.masked_fill(torch.logical_not(att_mask), float('-inf'))#[bs,n_h,T,1,O]
        att = F.softmax(att, dim=-1)#[bs,n_h,T,1,O]#由于存在所有obj都无效的情况，还需要再mask一次
        att=att.masked_fill(torch.logical_not(att_mask), float('0'))#[bs,n_h,T,1, obj]
        att = self.attn_drop(att)

        v_mask=att_mask.permute(0,1,2,4,3)#[bs,1,T,O,1]
        v_masked=v*v_mask
        out = torch.matmul(att,v_masked)#[bs,n_h,T,1,c_h]
        out = out.permute(0,3,1,4,2)#[bs,1,n_h,c_h,T]
        out=out.reshape(B,C,T)
        out = self.proj_drop(self.proj(out)) * mask.to(out.dtype)
        return out, mask

class ObjectTransformerBlock(nn.Module):
    """
    A simple (post layer norm) Transformer block
    Modified from https://github.com/karpathy/minGPT/blob/master/mingpt/model.py
    """
    ...
    def forward(self, x, mask,src_obj,src_obj_mask, cross_y=None, cross_y_mask=None, pos_embd=None):
        ...
        # print('obj_out:',obj_out.norm())
        out=self.obj_pool_skip(out) * out_mask_float + self.drop_path_attn(obj_out)
        # print('out:',out.norm())
        # optional cross_modal attention
        if self.use_cross_modal and cross_y is not None:
            # print("inside")
...
class ObjectQueryMaskedMHA(MaskedMHA):
...
class ObjectQueryTransformerBlock(nn.Module):
    """
    A simple (post layer norm) Transformer block
    Modified from https://github.com/karpathy/minGPT/blob/master/mingpt/model.py
    """
...
class ObjectShareTransformerBlock(nn.Module):
    """
    A simple (post layer norm) Transformer block
    Modified from https://github.com/karpathy/minGPT/blob/master/mingpt/model.py
    """
    ...
    def __init__(
        ...
        if path_pdrop > 0.0:
            ...
            self.drop_path_attn = nn.Identity()
            self.drop_path_mlp = nn.Identity()
    def share_obj_attn(self,x, mask, encoder_hidden_states, encoder_hidden_states_mask):
        # encoder_hidden_states=src_obj#[bs x maxlen,C,max_seq_len]因为只接2d和3d
        # encoder_hidden_states_mask=src_obj_mask#[bs x maxlen,1,max_seq_len]
        B, C, T = x.size()
        model=self.cross_attn
        q =model.query(x)
        k =model.key(encoder_hidden_states)
        v =model.value(encoder_hidden_states)

        q = q.view(B,    model.n_head, model.n_channels, -1).transpose(2, 3)#[bs,n_h,T,c_h]
        k = k.view(B, -1,model.n_head, model.n_channels, T)#[bs,maxobjlen,n_h,c_h,T]
        v = v.view(B, -1,model.n_head, model.n_channels, T)#[bs,maxobjlen,n_h,c_h,T]
        q=q*model.scale
        # att = torch.einsum('bntc,bonca->bonta',q,k)#[bs,obj,n_h,T,T]这里不对，不应该计算所有帧的物体注意力
        att = torch.einsum('bntc,bonct->bont',q,k)#[bs,obj,n_h,T]
        att_mask=encoder_hidden_states_mask.view(B,-1,1,T)#[bs,obj,1,T]
        ...
        att_mask=rearrange(att_mask,'b o 1 t -> b 1 t o')
        att=att.masked_fill(torch.logical_not(att_mask), float('0'))#[bs,n_h,T, obj]
        att = model.attn_drop(att)
        v_mask=rearrange(att_mask,'b 1 t o -> b o 1 1 t').to(v.dtype)
        v_masked=v*v_mask
        out = torch.einsum('bnto,bonct->bnct',att,v_masked)#[bs,n_h,c_h,T]
        out = out.reshape(B, C, -1)
        out = model.proj_drop(model.proj(out)) * mask.to(out.dtype)
        return out, mask
    def forward(self, x, mask,src_obj,src_obj_mask, cross_y=None, cross_y_mask=None, pos_embd=None):
        # pre-LN transformer: https://arxiv.org/pdf/2002.04745.pdf
        # print('x:',x.norm())
        ...
        # print('out:',out.norm())
        # object attention
        obj_out, _ = self.share_obj_attn(self.ln_obj(out), out_mask_float, self.ln_obj(src_obj), src_obj_mask)
        # print('obj_out:',obj_out.norm())
        out=self.obj_pool_skip(out) * out_mask_float + self.drop_path_attn(obj_out)
        # print('out:',out.norm())
        # optional cross_modal attention
...
class ObjectPlTransformerBlock(nn.Module):
    """
    A simple (post layer norm) Transformer block
    Modified from https://github.com/karpathy/minGPT/blob/master/mingpt/model.py
    """
...
class ObjectBMDTransformerBlock(nn.Module):
    """
    A simple (post layer norm) Transformer block
    Modified from https://github.com/karpathy/minGPT/blob/master/mingpt/model.py
    """
    ...
    def __init__(
        ...
        self.ln2 = LayerNorm(n_embd)
        # self.ln_obj_mlp = LayerNorm(n_embd)
        self.ln_concat_mlp = LayerNorm(n_embd*3)
        # specify the attention module
        if mha_win_size > 1:
            self.attn = LocalMaskedMHCA(
                n_embd,
    ...
    def forward(self, x, mask,src_obj,src_obj_mask, cross_y=None, cross_y_mask=None, pos_embd=None):
        ...
        # alpha=self.gate_mlp(self.ln_gate_mlp(torch.cat([obj_out,out],dim=1))* out_mask_float).sigmoid()#[bs,c,t]
        out=out+self.concat_mlp(self.ln_concat_mlp(torch.cat([out+src_obj,out*src_obj,torch.max(out,src_obj)],dim=1))* out_mask_float)
...
class LMDMaskedMHA(MaskedMHA):
    ...
    def __init__(
        ...
        super().__init__(n_embd, n_head, attn_pdrop=attn_pdrop, proj_pdrop=proj_pdrop)
...
class LMD(nn.Module):
    """
    A simple (post layer norm) Transformer cross encoder block
    Modified from PRVG
    """
    ...
    def forward(self, vid, mask, query=None, query_mask=None, pos_embd=None):
        # pre-LN transformer: https://arxiv.org/pdf/2002.04745.pdf

        #  downsample in the multi-head local attention


        query_mask_float=query_mask.to(query.dtype)
        cross_out, cross_out_mask,attn = self.cross_attn(self.ln3(query), query_mask_float, self.ln3(vid), mask)
        out_mask_float = query_mask.to(cross_out_mask.dtype)
        out = self.cross_pool_skip(query) * out_mask_float + self.drop_path_attn(cross_out)
        ...
        if pos_embd is not None:
            out += pos_embd * out_mask_float
        attn=attn.mean(dim=1).squeeze(1)#[B,T]
        return out, query_mask,attn
