...
@register_generator('dynamic_point')
class DynamicPointGenerator(nn.Module):
    """
        A generator for temporal "points"
    ...
    def forward(self,index):
        ...
        for l, stride in enumerate(self.fpn_strides):
            reg_range = torch.as_tensor(
                self.regression_range[l], dtype=torch.float)
            # fpn_stride = torch.as_tensor(stride, dtype=torch.float)
            # fpn_stride = fpn_stride[None].unsqueeze(0).repeat(bs,T, 1).to(index.device)
            points = index[:,::stride].unsqueeze(-1)#[bs,T,1]
            fpn_stride=points[:,1:]-points[:,:-1]
            fpn_stride=torch.cat((points[:,0].unsqueeze(-1)+1,fpn_stride),dim=1)#[bs,T,1]
            bs,T,_=points.shape
            # points = torch.arange(0, self.max_seq_len, stride)[:, None]
            # add offset if necessary (not in our current model)
            if self.use_offset:
                points += 0.5 * stride
            # pad the time stamp with additional regression range / stride
            reg_range =  reg_range[None].unsqueeze(0).repeat(bs,T, 1).to(index.device)
